...
function _M.rewrite(conf, ctx)
    ...
    if conf.https_to_http and _scheme == "https" then
        if ret_port == nil or ret_port == 80 or ret_port <= 0 or ret_port > 65535  then
            uri = "http://$host$request_uri"
        else
            -- uri = "http://$host:" .. ret_port .. "$request_uri"
            uri = "http://$host$request_uri"
        end
        local method_name = ngx.req.get_method()
        if method_name == "GET" or method_name == "HEAD" then
